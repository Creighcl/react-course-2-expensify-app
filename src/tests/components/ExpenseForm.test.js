...
test('should not set amount if invalid input', () => {
    const value = '12.122';
    const wrapper = shallow(<ExpenseForm />);
    const initValue = wrapper.state('amount');
    wrapper.find('input').at(1).simulate('change', {
        target: { value }
    });
    expect(initValue).not.toBe(value);
    expect(wrapper.state('amount')).toBe(initValue);
});
...
test('should set calendar focus onChange', () => {
    ...
    expect(wrapper.state('createdAtFocused')).toBe(true);
    wrapper.find('SingleDatePicker').prop('onFocusChange')({focused: false });
    expect(wrapper.state('createdAtFocused')).toBe(false);
});